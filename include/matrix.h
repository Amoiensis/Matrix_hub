...
static MATRIX_TYPE M_fro_norm_raw(Matrix *_mat) {
    MATRIX_TYPE norm = 0;
    int i, size;
    if ((_mat == NULL) || (_mat->data == NULL)) {
        return 0;
    }
    size = _mat->row * _mat->column;
    for (i = 0; i < size; i++) {
        norm = hypot(norm, _mat->data[i]);
    }
    return norm;
}

static MATRIX_TYPE M_relative_tolerance(MATRIX_TYPE scale) {
    scale = fabs(scale);
    return _MATRIX_ABS_EPS_ + _MATRIX_REL_EPS_ * scale;
}

static MATRIX_TYPE M_matrix_tolerance(Matrix *_mat) {
    /* Use a Frobenius scale for simple educational algorithms. It is cheap,
     * easy to explain, and avoids applying the same absolute cutoff to all
     * matrix magnitudes. */
    return M_relative_tolerance(M_fro_norm_raw(_mat));
}

static MATRIX_TYPE M_scalar_tolerance(MATRIX_TYPE value) {
    return M_relative_tolerance(value);
}
 ...
 * (函数: M_rank) 释放初等变换内存空间*/
    ...
    while (temp_Etrans != NULL) {
        Etrans_struct *prev_Etrans = temp_Etrans->forward_E_trans;
        free(temp_Etrans);
        temp_Etrans = prev_Etrans;
    }
    return 0;
}
 ...
 * Rank of the Matrix
 * 矩阵的秩 */
    (_DETAILED_>=2)?printf(">>rank(Matrix_%p)=\n... => transform...\n", (void *)_mat):0;
 ...
 * 可以调用如, help("help")，查看help函数的使用方法和内容*/
    ...
    printf("%s", file_name);
    printf(")\n");
    char temp_route[_MAX_HELP_LENGTH_];
    snprintf(temp_route, sizeof(temp_route), "../help/%s.txt", file_name);
    FILE *fp;
    char ch;
 ...
 * 导入_生成矩阵*/
    if ((row <= 0) || (column <= 0)) {
        printf("@ERROR: Matrix_gen Wrong!\n\tDetails: row and column must be positive.\n");
        return NULL;
    }
    Matrix *_mat = (Matrix *) malloc(sizeof(Matrix));
    if (_mat == NULL) {
        printf("@ERROR: Matrix_gen malloc Matrix failed!\n");
        return NULL;
    }
    _mat->row = row;
    _mat->column = column;
    ...
    if (_mat->data == NULL) {
        printf("@ERROR: Matrix_gen malloc data failed!\n");
        free(_mat);
        return NULL;
    }
//    using memory-copy to speed up.
    if (data != NULL) {
        ...
        memset(_mat->data, 0, (size) * sizeof(MATRIX_TYPE));
    }
//    int i;
//    for (i = 0; i < size; i++) {
//        _mat->data[i] = data[i];
 ...
 * 矩阵加减法 */
    ...
    if ((_mat_subed == NULL) || (_mat_minus == NULL)) {
 ...
 * 矩阵乘法 */
    (_DETAILED_>=3)?printf(">>Matrix_%p * Matrix_%p =\n", (void *)_mat_left, (void *)_mat_right):0;
    /*Determine_Matrix_Dimensions*/
    Matrix *_mat_result = NULL;
    if ((_mat_left == NULL) || (_mat_right == NULL)) {
        printf(M_mul_001);
        return NULL;
    }
 ...
 * 上三角化*/
    ...
    if (_Etrans_temp_head != NULL) {
        free(_Etrans_temp_head);
    }
    Uptri_struct *_Uptri = (Uptri_struct *) malloc(sizeof(Uptri_struct));
    _Uptri->trans_matrix = trans_mat;
    _Uptri->Uptri_matrix = _mat;
    _Uptri->swap_count = swap_count;
    (_DETAILED_>=1)?printf(">>Uptri(Matrix_%p)=\n", (void *)_mat_source):0;
    (_DETAILED_>=1)?printf("\tMatrix_%p * Matrix_%p\n", (void *)trans_mat, (void *)_mat):0;
    return _Uptri;
}
 ...
 * 下三角化*/
    ...
    (_DETAILED_>=1)?printf(">>Lowtri(Matrix_%p)=\n", (void *)_mat_source):0;
    (_DETAILED_>=1)?printf("\tMatrix_%p * Matrix_%p\n", (void *)_mat, (void *)trans_mat):0;
    return _Lowtri;
}
 ...
 * 对角矩阵求逆*/
    ...
    if (_mat_source->row != _mat_source->column) {
        ...
        MATRIX_TYPE tol = M_matrix_tolerance(_mat_source);
        int i, order = _mat_source->column;
        for (i = 0; i < order; i++) {
            if(fabs((data)[i * (order + 1)]) <= tol){ // 不可逆
 ...
 * 对角化*/
    ...
    (_DETAILED_>=1)?printf(">>Diag(Matrix_%p)=\n", (void *)_mat_source):0;
    (_DETAILED_>=1)?printf("\tMatrixdl%p * Matrix_d%p * Matrixr_%p\n", (void *)_Uptri_->trans_matrix,
                        (void *)_Lowtri_->Lowtri_matrix, (void *)_Lowtri_->trans_matrix):0;
    // 释放内存
 ...
 * Inverse (create).
 * 矩阵求逆*/
    if ((_mat == NULL) || (_mat->row != _mat->column)) {
        printf(M_Dia_Inv_002);
        return NULL;
    }
    if (_mat->row == 1) {
        if (fabs(_mat->data[0]) <= M_scalar_tolerance(_mat->data[0])) {
            printf(M_Dia_Inv_023);
            return NULL;
        }
        MATRIX_TYPE data = 1 / _mat->data[0];
        return Matrix_gen(1, 1, &data);
    }
    M_inv_struct *_Uptri_ = M_Uptri_4inv(_mat);
    M_inv_struct *_Lowtri_ = M_Lowtri_4inv(_Uptri_->_matrix);
    Matrix *_mat_dia_inv = M_Dia_Inv(_Lowtri_->_matrix);
    if (_mat_dia_inv == NULL) {
        M_free(_Uptri_->_matrix);
    ...
    (_DETAILED_>=2)?printf(">>Inv(Matrix_%p)=\n", (void *)_mat):0;
    (_DETAILED_>=2)?printf("\tMatrix_inv_%p\n", (void *)_mat_inv):0;
    // 释放内存
    M_free(_Uptri_->_matrix);
    M_free(_Lowtri_->_matrix);
    free(_Uptri_);
 ...
 * 释放矩阵，释放内存*/
    if (_mat == NULL){
        (_DETAILED_>=3)?printf(">>M_free(NULL): skipped.\n"):0;
        return 0;
    }
    free(_mat->data);
    (_DETAILED_>=3)?printf(">>Matrix_%p has been freed.\n", (void *)_mat):0;
    free(_mat);
    return 0;
}
...
//                                                                                (j - column_left)];
    ...
    (_DETAILED_>=2)?(printf(">>Full<U%d,D%d,L%d,R%d>(Matrix_%p)=\n\tMatrix_%p\n",
                         row_up, row_down, column_left, column_right, (void *)_mat, (void *)mat_result)):0;
    return mat_result;
}
    ...
    if (row == _ONE_ || column == _ONE_) {/*向量的范数*/
        ...
        switch (Setting) {
            ...
            case 2: {/*矩阵的2范数*/
                ...
                if (M_temp_1_eigen != NULL) {
                    Val_norm = sqrt(fabs(M_temp_1_eigen->eigen_value));
 ...
 * Reference: [幂法-算法参考](https://max.book118.com/html/2017/0527/109650252.shtm) */
    ...
    if ((_mat != NULL) && (_mat->column == _mat->row)) {
        Matrix *mat_z = M_Ones(_mat->column, 1), *mat_z_next = NULL;
        Matrix *mat_y = NULL, *mat_z_gap = NULL;
        MATRIX_TYPE m_value = 0, mat_z_gap_norm = 1, max_abs = 0;
        MATRIX_TYPE zero_tol = M_matrix_tolerance(_mat);
        MATRIX_TYPE deta = _MATRIX_ITER_EPS_; // 相对迭代精度
        int i, temp_num = 0, epoch = 0, size = _mat->column;

        while ((mat_z_gap_norm > deta) && (epoch < _MAX_LOOP_NUM_)) {
            mat_y = M_mul(_mat, mat_z);
            max_abs = 0;
            for (i = 0; i < size; i++) {
                if (fabs(mat_y->data[i]) > max_abs) {
                    max_abs = fabs(mat_y->data[i]);
                    temp_num = i;
                }
            }
            if (max_abs <= zero_tol) {
            ...
            mat_z_next = Matrix_copy(mat_y);
            M_numul(mat_z_next, 1 / m_value);
            mat_z_gap = M_add_sub(1, mat_z_next, 1, mat_z);
 ...
 * 2. [求解-特征向量参考](https://blog.csdn.net/w_weixiaotao/article/details/111868086) */
    ...
    if ((_mat != NULL) && (_mat->column == _mat->row)) {
        ...
        for(eigen_count=0;eigen_count<dim;eigen_count++){
            ...
            MATRIX_TYPE sum1 = 1;
            // Note: 感谢 645770225, 具体参考[issue-10](https://github.com/Amoiensis/Matrix_hub/issues/10)
            if (fabs(_mat_->data[(dim - 1) * dim + (dim - 1)]) > zero_tol){
                // 如果矩阵最后一行不为0
                sum1 = 0;
            ...
            for (ik = dim - 2; ik >= 0; ik--){
                ...
                if (fabs(_mat_->data[ik * dim + ik]) > zero_tol){
                    // 如果对角线元素不为0
 ...
 */
    ...
    x_norm = M_norm(_x, 2);
    tol = M_matrix_tolerance(_x);
    if (x_norm <= tol){
        H = M_I(_x->row);
        return H;
    }

    /* Stable reflector vector:
     * alpha = -sign(x0) * ||x||, v = x - alpha * e1.
     * This avoids cancellation when x is already close to e1.
     */
    w = Matrix_copy(_x);
    if (w == NULL) {
        return NULL;
    }
    alpha = (_x->data[0] >= 0) ? -x_norm : x_norm;
    w->data[0] -= alpha;
    w_norm = M_norm(w, 2);
    if (w_norm <= tol) {
        M_free(w);
 ...
 * */
    ...
    /* Apply Householder vectors directly. This keeps the implementation
     * readable, supports rectangular matrices, and avoids constructing a full
     * Hi/Qi matrix at every step. Q is still returned explicitly for the simple
     * public API and for script multi-output usage. */
    for(k=0;k<limit;k++){
        int len = m - k;
        MATRIX_TYPE *v = (MATRIX_TYPE *)malloc(sizeof(MATRIX_TYPE) * len);
        MATRIX_TYPE x_norm = 0;
        MATRIX_TYPE v_norm = 0;
        MATRIX_TYPE alpha;
        if (v == NULL) {
            M_free(Q);
            M_free(R);
        ...
        alpha = (v[0] >= 0) ? -x_norm : x_norm;
        v[0] -= alpha;
        for(i=0;i<len;i++){
            v_norm = hypot(v_norm, v[i]);
        }
        if (v_norm <= tol) {
            free(v);
            continue;
        ...
        for(i=0;i<m;i++){
            MATRIX_TYPE dot = 0;
            for(t=0;t<len;t++){
                dot += Q->data[i * m + (k + t)] * v[t];
            }
            for(t=0;t<len;t++){
                Q->data[i * m + (k + t)] -= 2 * dot * v[t];
            }
        }
 ...
 */
    ...
    MATRIX_TYPE eigen_tol = M_matrix_tolerance(_mat_T_mat);
    for (i=0;i<Dia_len;i++){
        MATRIX_TYPE sigma2 = M_eigen_val_vec_V[val]->data[i];
        if ((sigma2 < 0) && (fabs(sigma2) <= eigen_tol)) {
            sigma2 = 0;
        } else if (sigma2 < 0) {
            sigma2 = fabs(sigma2);
        }
        M_eigen_val_vec_V[val]->data[i] = sqrt(sigma2);
    }
    mat_list_SVD[Dia] = M_eigen_val_vec_V[val];
    M_free(_mat_T_mat);
    free(M_eigen_val_vec_V);
    Matrix * _mat_mat_T = M_mul(_mat, _mat_T);
    Matrix ** M_eigen_val_vec_U = M_eigen(_mat_mat_T);
    if (M_eigen_val_vec_U == NULL) {
        M_free(_mat_mat_T);
        M_free(_mat_T);
    ...
    * mat_svd_inv_dia_T = NULL, * mat_temp_1 = NULL, * mat_temp_2 = NULL;
    ...
    switch (_method_) {
        ...
        case _SVD_:
            ...
            if (mat_list_SVD == NULL) {
                return NULL;
            }
            mat_svd_inv_dia_T = M_Zeros(_mat->column, _mat->row);
            enum{U=0, Dia=1, V=2};
            int sigma_count = (mat_svd_inv_dia_T->row < mat_svd_inv_dia_T->column) ?
                              mat_svd_inv_dia_T->row : mat_svd_inv_dia_T->column;
            int dia_count = mat_list_SVD[Dia]->row * mat_list_SVD[Dia]->column;
            MATRIX_TYPE sigma_max = 0;
            MATRIX_TYPE sigma_tol = 0;
            if (dia_count < sigma_count) {
                sigma_count = dia_count;
            }
            for(i=0;i<sigma_count;i++){
                if (fabs(mat_list_SVD[Dia]->data[i]) > sigma_max) {
                    sigma_max = fabs(mat_list_SVD[Dia]->data[i]);
                }
            }
            sigma_tol = _MATRIX_ABS_EPS_ + _MATRIX_REL_EPS_ *
                        ((_mat->row > _mat->column) ? _mat->row : _mat->column) * sigma_max;
            for(i=0;i<sigma_count;i++){
                val_temp = mat_list_SVD[Dia]->data[i];
                if (fabs(val_temp) > sigma_tol){
                    mat_svd_inv_dia_T->data[i*(mat_svd_inv_dia_T->column)+i] = 1/(val_temp);
                }else{
                    mat_svd_inv_dia_T->data[i*(mat_svd_inv_dia_T->column)+i] = 0;
                }
            }
