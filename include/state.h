 ...
 * The algorithms are tuned for double precision. Changing MATRIX_TYPE to
 * float can make inverse/decomposition results unstable on ill-conditioned
 * matrices; see GitHub issue #14 before using float in MCU projects.
 */
...
/* Numerical tolerances - 数值阈值
 * `_APPROXIMATELY_ZERO_` is kept as a small absolute fallback. Matrix
 * algorithms should prefer a relative tolerance such as:
 *     _MATRIX_ABS_EPS_ + _MATRIX_REL_EPS_ * ||A||
 * This keeps tiny and huge matrices from sharing the old fixed 1e-5 cutoff.
 */
#define _MATRIX_ABS_EPS_ (1e-12)
#define _MATRIX_REL_EPS_ (1e-10)
#define _MATRIX_ITER_EPS_ (1e-10)
#define _APPROXIMATELY_ZERO_ (_MATRIX_ABS_EPS_)
...
#define M_QR_029    "@ERROR: M_QR Wrong!\n\tDetails:(M_QR_029)QR decomposition requires a non-null matrix.\n"
